...
func (p localProvider) renderLoginPage(c echo.Context, reason string) error {
	...
	return templates.Templates.ExecuteTemplate(c.Response(), "local-login.html", context)
}
...
func (p *localProvider) handlePasswordPage(c echo.Context, session *Session) error {
	...
	return templates.Templates.ExecuteTemplate(c.Response(), "local-password-change.html", context)
}

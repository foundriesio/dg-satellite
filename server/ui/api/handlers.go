...
func RegisterHandlers(e *echo.Echo, storage *storage.Storage, a auth.Provider) {
	...
	g.GET("/devices/:uuid/config/apps", h.deviceConfigAppsGet, requireScope(users.ScopeDevicesR))
	g.GET("/devices/:uuid/config/apps-available", h.deviceConfigAppsAvailableGet, requireScope(users.ScopeDevicesR))
	g.PUT("/devices/:uuid/config/apps", h.deviceConfigAppsPut, requireScope(users.ScopeDevicesRU))

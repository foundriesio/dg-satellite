...
func (p *oauth2BaseProvider) configure(e *echo.Echo, usersStorage *users.Storage, cfg *storage.AuthConfig) error {
	...
	p.oauthConfig = &oauth2.Config{
		RedirectURL:  cfgOauth.BaseUrl + "/auth/callback",
		ClientID:     cfgOauth.ClientID,
		ClientSecret: cfgOauth.ClientSecret,
		Scopes:       p.scopes,
		Endpoint:     p.endpoint,
	}
	...
	e.GET("/auth/login", p.handleLogin)
...
func (p oauth2BaseProvider) GetSession(c echo.Context) (*Session, error) {
	...
	if err != nil {
		...
		return nil, p.renderLoginPage(c, "No cookie found")
	}
	...
	return nil, p.renderLoginPage(c, "Session not found")
}

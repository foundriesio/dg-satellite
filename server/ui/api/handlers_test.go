...
func NewTestClientWithCA(t *testing.T, org string) *testClient {
	...
	if len(org) > 0 {
		...
		fsS.Certs.WriteFile(storage.CertsCaKeyFile, caKeyPem)
		fsS.Certs.WriteFile(storage.CertsCaPemFile, caCertPem)
		fsS.Certs.WriteFile(storage.CertsCaRootFile, caCertPem) // doesn't matter for tests, just needs to exist
		...
		fsS.Certs.WriteFile(storage.CertsTlsPemFile, tlsCertPEM)

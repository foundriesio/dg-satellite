...
func (d *Device) Tests() ([]storage.TargetTest, error) {
	...
	files, err := d.storage.fs.ListFiles(d.Uuid, storage.TestsPrefix, true)
	...
	for _, file := range files {
		testData, err := d.storage.fs.ReadFile(d.Uuid, file)
		if err != nil {
			return nil, err
		}
...
func (d *Device) Test(testId string) (*storage.TargetTest, error) {
	...
	var test storage.TargetTest
	if err := d.storage.fs.ReadAsJson(d.Uuid, name, &test); err != nil {
	...
	files, err := d.storage.fs.ListFiles(d.Uuid, prefix, true)
	if err != nil {
...
func (d *Device) TestArtifact(testId, name string) (io.ReadCloser, error) {
	...
	fd, err := d.storage.fs.ReadFileStream(d.Uuid, name)
	if err != nil && errors.Is(err, os.ErrNotExist) {
		err = nil
	}
	return fd, err

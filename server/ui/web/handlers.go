...
func RegisterHandlers(e *echo.Echo, storage *users.Storage, authProvider auth.Provider) {
	...
	e.GET("/auth/confirm-activation", h.authDeviceConfirm, h.requireSession)
	e.POST("/auth/authorize-activation", h.authDeviceAuthorize, h.requireSession)
	e.POST("/auth/deny-activation", h.authDeviceDeny, h.requireSession)

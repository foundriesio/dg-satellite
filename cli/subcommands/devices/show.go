...
var showCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		return showDevice(api, args[0], aktoml, hwinfo)
	},
}
...
func showDevice(api *api.Api, uuid string, aktoml, hwinfo bool) error {
	var device rest.Device
	err := api.Get(fmt.Sprintf("/v1/devices/%s", uuid), &device)
	cobra.CheckErr(err)

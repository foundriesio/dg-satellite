...
func NewTestClientWithCA(t *testing.T, org string) *testClient {
	...
	if len(org) > 0 {
		...
		require.Nil(t, fsS.Certs.WriteFile(storage.CertsCaKeyFile, caKeyPem))
		require.Nil(t, fsS.Certs.WriteFile(storage.CertsCaPemFile, caCertPem))
		require.Nil(t, fsS.Certs.WriteFile(storage.CertsCaRootFile, caCertPem)) // doesn't matter for tests, just needs to exist
